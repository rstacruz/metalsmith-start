...
var c = {
  accent1: cliColor.blackBright, // subtitle
  accent2: cliColor.blue, // build text
  headline: cliColor.bold, // main title
  url: cliColor.underline,
  ok: cliColor.green,
  red: cliColor.red,
  mute: cliColor.blackBright
}
...
Reporter.prototype = {
  ...
  buildTo: function (files) {
    ...
    if (cliColor.windowSize.width > 100 && this.lastTask) {
...
// Monkey-patch for https://github.com/dylang/observatory/pull/6
out.height = function height (text) {
    return Math.ceil(out.ln(text) / cliColor.windowSize.width)
}
...
function customRender () {
  ...
  var output =
    ...
    (task.detailsLabel.toString().length ? (
      out.padding(settings.width() - out.ln(statusLabel) - out.ln(task.description) - 2) +
      ' ' +
      task.detailsLabel) : '')
