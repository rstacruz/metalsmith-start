...
Reporter.prototype = {
  ...
  start: function (banner, port) {
    this.add('')
    this.add(ch.bold(banner))
    this.add(ch.gray('starting ' + process.env.NODE_ENV + ' - ^C to exit'))
    this.add('')
    ...
    this.tasks = {
      ...
      serve: this.add('› ' + ch.underline('http://localhost:' + port)).status(symbols.wait),
      _: this.add(''),
      status: this.add(ch.gray('Starting up...'))
    }
  },
