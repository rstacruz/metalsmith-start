...
Runner.prototype.enableLR = unyield(function * () {
  ...
  function onChange (files) {
    var fname = require('path').join(ms._destination, files[0])
    var prefix = '->: ' + eventSymbols.change + ' '
    if (files.length === 1) {
      this.log(prefix + fname)
    } else {
      var n = files.length - 1
      var others = n === 1 ? '1 other' : (n + ' others')
      this.log(prefix + fname + ' (and ' + others + ')')
    }
  }
})
