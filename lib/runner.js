...
Runner.prototype.start = unyield(function * () {
  ...
  obs.add(chalk.bold(this.banner) + '  ' +
    chalk.gray('' + process.env.NODE_ENV + '...'))
  obs.add('')
  this.tasks = {
    build: obs.add('› first build').status('starting...'),
    watch: obs.add('› watching updates').status('starting...'),
    livereload: obs.add('› livereload').status('starting...'),
    serve: obs.add('› serve').status('starting...'),
  }
...
Runner.prototype.serve = unyield(function * () {
  ...
  task.details(chalk.underline('http://localhost:' + this.port))
  task.done('on ✓')
  task.update()
  obs.add('')
})
